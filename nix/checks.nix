...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      rhemify = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "rhemify-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${rhemifyVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "rhemify-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${rhemify}/bin/rhemify || (echo "FAIL: rhemify binary missing"; exit 1)
          test -x ${rhemify}/bin/rhemify || (echo "FAIL: rhemify binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${rhemify}/bin/rhemify version 2>&1 | grep -qi "rhemify" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "rhemify-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in rhemify rhemify rhemify-acp; do
            test -x ${rhemify}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "rhemify-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking rhemify --help ==="
          ${rhemify}/bin/rhemify --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${rhemify}/bin/rhemify --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "rhemify-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${rhemify}/share/rhemify/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${rhemify}/share/rhemify/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "RHEMIFY_BUNDLED_SKILLS" ${rhemify}/bin/rhemify || \
            (echo "FAIL: RHEMIFY_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: RHEMIFY_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify RHEMIFY_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "rhemify-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(RHEMIFY_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking RHEMIFY_MANAGED guards ==="
          check_blocked "config set" ${rhemify}/bin/rhemify config set model foo
          check_blocked "config edit" ${rhemify}/bin/rhemify config edit
        ...
        in pkgs.runCommand "rhemify-config-roundtrip" {
          ...
          merge_and_load() {
            local rhemify_home="$1"
            export RHEMIFY_HOME="$rhemify_home"
            ${configMergeScript} ${nixSettings} "$rhemify_home/config.yaml"
            ${rhemifyVenv}/bin/python3 -c '
import json, sys
