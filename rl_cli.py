...
if tinker_atropos_dir.exists():
    ...
    os.environ['RHEMIFY_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to rhemify directory if submodule not found
...
from rhemify_constants import get_rhemify_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_rhemify_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.rhemify/config.yaml if not provided)
    ...
    """
    # Load config from ~/.rhemify/config.yaml
    config = load_rhemify_config()
