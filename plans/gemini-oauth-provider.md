...
## Client ID
- Need to register a "Desktop app" OAuth client on a the project GCP project
...
## Token Lifecycle
- Store at `~/.rhemify/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `rhemify_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `rhemify_cli/models.py` — Add Gemini model catalog
4. `rhemify_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `rhemify_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `rhemify_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- the project GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via RHEMIFY_GEMINI_CLIENT_ID env var
...
- rhemify Copilot OAuth: `rhemify_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
