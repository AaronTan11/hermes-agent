...
_SENSITIVE_RHEMIFY_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    rhemify_home = Path(
        os.getenv("RHEMIFY_HOME", str(home / ".rhemify"))
    ).expanduser().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(rhemify_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(rhemify_home / rel for rel in _SENSITIVE_RHEMIFY_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Rhemify path and cannot be attached")
