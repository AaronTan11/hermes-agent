...
class MemoryManager:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``rhemify_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_rhemify_home()`` themselves.
        """
        if "rhemify_home" not in kwargs:
            from rhemify_constants import get_rhemify_home
            kwargs["rhemify_home"] = str(get_rhemify_home())
        for provider in self._providers:
            try:
