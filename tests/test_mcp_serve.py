"""
Tests for mcp_serve — Rhemify MCP server.
...
def _isolate_rhemify_home(tmp_path, monkeypatch):
    """Redirect RHEMIFY_HOME to a temp directory."""
    monkeypatch.setenv("RHEMIFY_HOME", str(tmp_path))
    try:
        import rhemify_constants
        monkeypatch.setattr(rhemify_constants, "get_rhemify_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking rhemify_state schema."""
