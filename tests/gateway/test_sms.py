...
class TestSmsToolset:
    def test_rhemify_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("rhemify-sms")
        assert ts is not None
        assert "tools" in ts

    def test_rhemify_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("rhemify-gateway")
        assert gw is not None
        assert "rhemify-sms" in gw["includes"]
