...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "rhemify"
        assert config.workspace_id == "rhemify"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"rhemify": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"rhemify": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"rhemify": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="rhemify"
        ):
            result = config.resolve_session_name("/home/user/rhemify/subdir")
        assert result == "rhemify"
...
class TestResolveConfigPath:
    def test_prefers_rhemify_home_when_exists(self, tmp_path):
        rhemify_home = tmp_path / "rhemify"
        rhemify_home.mkdir()
        local_cfg = rhemify_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        rhemify_home = tmp_path / "rhemify"
        rhemify_home.mkdir()
        # No honcho.json in RHEMIFY_HOME — also isolate ~/.rhemify so
    ...
    def test_falls_back_to_global_without_rhemify_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("RHEMIFY_HOME", None)
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH
...
class TestResolveActiveHost:
    def test_default_returns_rhemify(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("RHEMIFY_HONCHO_HOST", None)
            os.environ.pop("RHEMIFY_HOME", None)
            assert resolve_active_host() == "rhemify"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"RHEMIFY_HONCHO_HOST": "rhemify.coder"}):
            assert resolve_active_host() == "rhemify.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("RHEMIFY_HONCHO_HOST", None)
            with patch("rhemify_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "rhemify.coder"

    def test_default_profile_returns_rhemify(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("RHEMIFY_HONCHO_HOST", None)
            with patch("rhemify_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "rhemify"

    def test_custom_profile_returns_rhemify(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("RHEMIFY_HONCHO_HOST", None)
            with patch("rhemify_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "rhemify"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("RHEMIFY_HONCHO_HOST", None)
            # Temporarily remove rhemify_cli.profiles to simulate import failure
            saved = sys.modules.get("rhemify_cli.profiles")
            sys.modules["rhemify_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "rhemify"
            finally:
                if saved is not None:
                    sys.modules["rhemify_cli.profiles"] = saved
                else:
                    sys.modules.pop("rhemify_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="rhemify.coder")
        assert config.host == "rhemify.coder"
        assert config.workspace_id == "rhemify"  # shared workspace
        assert config.ai_peer == "rhemify.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="rhemify")
        assert config.host == "rhemify"
        assert config.workspace_id == "rhemify"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "rhemify": {"aiPeer": "rhemify", "peerName": "alice"},
                "rhemify.coder": {
                    "aiPeer": "rhemify.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="rhemify.coder", config_path=config_file,
        )
        assert config.host == "rhemify.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "rhemify.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="rhemify.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "rhemify.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"rhemify": {"enabled": True, "aiPeer": "rhemify"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"rhemify": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
