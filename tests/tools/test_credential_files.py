...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        (rhemify_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"RHEMIFY_HOME": str(rhemify_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(rhemify_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.rhemify/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        (rhemify_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        (rhemify_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        (rhemify_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.rhemify/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        rhemify_home = tmp_path / ".rhemify"
        (rhemify_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"RHEMIFY_HOME": str(rhemify_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.rhemify")

        assert mounts[0]["container_path"] == "/home/user/.rhemify/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.rhemify/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.rhemify/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape RHEMIFY_HOME."""
        monkeypatch.setenv("RHEMIFY_HOME", str(tmp_path / ".rhemify"))
        (tmp_path / ".rhemify").mkdir()

        # Create a sensitive file one level above rhemify_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        monkeypatch.setenv("RHEMIFY_HOME", str(rhemify_home))

        # Create a fake sensitive file outside rhemify_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside RHEMIFY_HOME must still be registered."""
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        monkeypatch.setenv("RHEMIFY_HOME", str(rhemify_home))
        (rhemify_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_rhemify_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of RHEMIFY_HOME must be allowed."""
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        subdir = rhemify_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside RHEMIFY_HOME pointing outside must be rejected."""
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        monkeypatch.setenv("RHEMIFY_HOME", str(rhemify_home))

        # Create a sensitive file outside rhemify_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside rhemify_home pointing outside
        symlink = rhemify_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes RHEMIFY_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, rhemify_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape RHEMIFY_HOME."""
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        monkeypatch.setenv("RHEMIFY_HOME", str(rhemify_home))
        ...
        sensitive.write_text("{}")
        self._write_config(rhemify_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(rhemify_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside RHEMIFY_HOME via config must still mount."""
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        monkeypatch.setenv("RHEMIFY_HOME", str(rhemify_home))

        (rhemify_home / "oauth.json").write_text("{}")
        self._write_config(rhemify_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        (rhemify_home / "cache" / "documents").mkdir(parents=True)
        (rhemify_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("RHEMIFY_HOME", str(rhemify_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.rhemify/cache/documents" in paths
        assert "/root/.rhemify/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.rhemify/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        rhemify_home = tmp_path / ".rhemify"
        rhemify_home.mkdir()
        # Use legacy dir name — get_rhemify_dir prefers old if it exists
        (rhemify_home / "document_cache").mkdir()
        (rhemify_home / "image_cache").mkdir()
        monkeypatch.setenv("RHEMIFY_HOME", str(rhemify_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(rhemify_home / "document_cache") in host_paths
        assert str(rhemify_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.rhemify/cache/documents" in container_paths
        assert "/root/.rhemify/cache/images" in container_paths

    def test_empty_rhemify_home(self, tmp_path, monkeypatch):
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        rhemify_home = tmp_path / ".rhemify"
        doc_dir = rhemify_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        rhemify_home = tmp_path / ".rhemify"
        ss_dir = rhemify_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.rhemify/cache/screenshots/session_abc/screen1.png"
