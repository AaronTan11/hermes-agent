...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._rhemify_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_rhemify_bin(self):
        """After _INSTALL_FAILED, manual install in RHEMIFY_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        rhemify_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(rhemify_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(rhemify_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._rhemify_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == rhemify_bin
            assert _tirith_mod._resolved_path == rhemify_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# RHEMIFY_HOME isolation
# ---------------------------------------------------------------------------

class TestRhemifyHomeIsolation:
    def test_rhemify_bin_dir_respects_rhemify_home(self):
        """_rhemify_bin_dir must use RHEMIFY_HOME, not hardcoded ~/.rhemify."""
        from tools.tirith_security import _rhemify_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"RHEMIFY_HOME": tmpdir}):
            result = _rhemify_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_rhemify_home(self):
        """_failure_marker_path must use RHEMIFY_HOME, not hardcoded ~/.rhemify."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"RHEMIFY_HOME": "/custom/rhemify"}):
            result = _failure_marker_path()
        assert result == "/custom/rhemify/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets RHEMIFY_HOME; verify it's active."""
        rhemify_home = os.getenv("RHEMIFY_HOME")
        assert rhemify_home is not None, "RHEMIFY_HOME should be set by conftest"
        assert "rhemify_test" in rhemify_home, "Should point to test temp dir"

    def test_get_rhemify_home_fallback(self):
        """Without RHEMIFY_HOME set, falls back to ~/.rhemify."""
        from tools.tirith_security import _get_rhemify_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove RHEMIFY_HOME entirely
            os.environ.pop("RHEMIFY_HOME", None)
            result = _get_rhemify_home()
        assert result == os.path.join(os.path.expanduser("~"), ".rhemify")
