...
class TestDelegationCredentialResolution(unittest.TestCase):
    ...
    @patch("rhemify_cli.runtime_provider.resolve_runtime_provider")
    ...
    def test_nous_provider_resolves_nous_credentials(self, mock_resolve):
        """Nous provider resolves Provider base_url and api_key."""
        mock_resolve.return_value = {
            "provider": "unused",
        ...
        parent = _make_mock_parent(depth=0)
        cfg = {"model": "rhemify-3-llama-3.1-8b", "provider": "unused"}
        creds = _resolve_delegation_credentials(cfg, parent)
        self.assertEqual(creds["provider"], "unused")
        self.assertEqual(creds["base_url"], "https://inference-api.example.com/v1")
        self.assertEqual(creds["api_key"], "nous-agent-key-xyz")
        mock_resolve.assert_called_once_with(requested="unused")
