...
def _isolate(tmp_path, monkeypatch):
    """Isolate RHEMIFY_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal RhemifyCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = RhemifyCLI.__new__(RhemifyCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["rhemify-core"]
        obj.compact = False
