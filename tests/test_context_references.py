...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Rhemify Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_rhemify_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("RHEMIFY_HOME", str(tmp_path / ".rhemify"))

    rhemify_env = tmp_path / ".rhemify" / ".env"
    rhemify_env.parent.mkdir(parents=True)
    rhemify_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.rhemify/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
