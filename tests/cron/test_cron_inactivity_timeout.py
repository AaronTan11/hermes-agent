...
- Unlimited timeout (RHEMIFY_CRON_TIMEOUT=0)
- Backward compat: RHEMIFY_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """RHEMIFY_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """RHEMIFY_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("RHEMIFY_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """RHEMIFY_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("RHEMIFY_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("RHEMIFY_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_rhemify_time_importable(self):
        """rhemify_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _rhemify_now
        assert callable(_rhemify_now)

    def test_rhemify_constants_importable(self):
        """rhemify_constants should be importable from cron context."""
        from rhemify_constants import get_rhemify_home
        assert callable(get_rhemify_home)
