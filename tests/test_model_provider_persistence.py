"""Tests that provider selection via `rhemify model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated RHEMIFY_HOME with a minimal string-format config."""
    home = tmp_path / "rhemify"
    home.mkdir()
    ...
    env_file = home / ".env"
    env_file.write_text("")
    monkeypatch.setenv("RHEMIFY_HOME", str(home))
    # Clear env vars that could interfere
    monkeypatch.delenv("RHEMIFY_MODEL", raising=False)
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from rhemify_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from rhemify_cli.main import _model_flow_api_key_provider
        ...
        with patch("rhemify_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("rhemify_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from rhemify_cli.main import _model_flow_copilot
        from rhemify_cli.config import load_config

        with patch(
            "rhemify_cli.auth.resolve_api_key_provider_credentials",
            return_value={
                "provider": "copilot",
            ...
            "rhemify_cli.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "rhemify_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "rhemify_cli.auth.deactivate_provider",
        ):
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from rhemify_cli.main import _model_flow_copilot_acp
        from rhemify_cli.config import load_config

        with patch(
            "rhemify_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "rhemify_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_upstream_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        from rhemify_cli.main import _model_flow_api_key_provider
        from rhemify_cli.config import load_config

        monkeypatch.setenv("UPSTREAM_GO_API_KEY", "test-key")

        with patch("rhemify_cli.models.fetch_api_models", return_value=["upstream-go/kimi-k2.5", "upstream-go/minimax-m2.7"]), \
             patch("rhemify_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("rhemify_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "upstream-go", "upstream-go/kimi-k2.5")

        import yaml
        config = yaml.safe_load((config_home / "config.yaml").read_text()) or {}
        model = config.get("model")
        assert isinstance(model, dict)
        assert model.get("provider") == "upstream-go"
        assert model.get("default") == "kimi-k2.5"
        assert model.get("api_mode") == "chat_completions"

    def test_upstream_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        from rhemify_cli.main import _model_flow_api_key_provider
        from rhemify_cli.config import load_config
        ...
        (config_home / "config.yaml").write_text(
            ...
            "  base_url: https://upstream.ai/zen/go/v1\n"
            "  api_mode: chat_completions\n"
        )

        with patch("rhemify_cli.models.fetch_api_models", return_value=["upstream-go/kimi-k2.5", "upstream-go/minimax-m2.5"]), \
             patch("rhemify_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("rhemify_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
            _model_flow_api_key_provider(load_config(), "upstream-go", "kimi-k2.5")
