...
from rhemify_cli.auth import AuthError
from rhemify_cli import main as rhemify_main
...
def test_rhemify_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in RhemifyCLI.__init__")

    monkeypatch.setattr("rhemify_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("rhemify_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.RhemifyCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("rhemify_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("rhemify_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_turn_routing_uses_cheap_model_when_simple(monkeypatch):
    ...
    shell = cli.RhemifyCLI(model="anthropic/claude-sonnet-4", compact=True, max_turns=1)
    shell.provider = "openrouter"
    shell.api_mode = "chat_completions"
    shell.base_url = "https://openrouter.ai/api/v1"
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    monkeypatch.setenv("RHEMIFY_INFERENCE_PROVIDER", "openrouter")
    ...
    shell = cli.RhemifyCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("rhemify_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "rhemify_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.RhemifyCLI(compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    ...
    config = {
        "model": {"provider": "unused", "default": "claude-opus-4-6"},
        "tts": {"provider": "elevenlabs"},
        "browser": {"cloud_provider": "browser-use"},
    }

    monkeypatch.setattr(
        "rhemify_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "nous-token"},
    )
    monkeypatch.setattr(
        "rhemify_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "rhemify_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("rhemify_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None: "claude-opus-4-6")
    monkeypatch.setattr("rhemify_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("rhemify_cli.auth._update_config_for_provider", lambda provider, url: None)
    monkeypatch.setattr(
        "rhemify_cli.nous_subscription.get_nous_subscription_explainer_lines",
        lambda: ["subscription enables managed web tools."],
    )

    rhemify_main._model_flow_nous(config, current_model="claude-opus-4-6")

    out = capsys.readouterr().out
    assert "subscription enables managed web tools." in out
...
def test_model_flow_nous_applies_managed_tts_default_when_unconfigured(monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "subscription enables managed web tools." in out
    assert "OpenAI TTS via your subscription" in out
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.RhemifyCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.RhemifyCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "rhemify_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("rhemify_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("rhemify_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("rhemify_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("rhemify_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(rhemify_main, "_prompt_provider_choice", lambda choices: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    rhemify_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "rhemify_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("rhemify_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("rhemify_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("rhemify_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("rhemify_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "rhemify_cli.models.probe_api_models",
    ...
    rhemify_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(rhemify_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "rhemify_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "unused"}},
    )
    monkeypatch.setattr("rhemify_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("rhemify_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("rhemify_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("rhemify_cli.auth.resolve_provider", lambda requested, **kwargs: "unused")
    monkeypatch.setattr("rhemify_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(rhemify_main, "_prompt_provider_choice", lambda choices: 0)
    ...
    monkeypatch.setattr("rhemify_cli.auth._login_nous", _fake_login)

    rhemify_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.example.com",
            inference_url="https://inference.example.com/v1",
            client_id="rhemify-local",
