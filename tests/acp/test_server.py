"""Tests for acp_adapter.server — RhemifyACPAgent ACP server."""
...
from acp_adapter.server import RhemifyACPAgent, RHEMIFY_VERSION
...
def agent(mock_manager):
    """RhemifyACPAgent backed by a mock session manager."""
    return RhemifyACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "rhemify"
        assert resp.agent_info.version == RHEMIFY_VERSION
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert RHEMIFY_VERSION in result
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("rhemify_cli.config.load_config", lambda: {
        ...
        with patch("run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = RhemifyACPAgent(session_manager=manager)
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["rhemify-acp"]
