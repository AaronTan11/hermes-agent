...
import rhemify_cli.gateway as gateway_cli


class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "rhemify-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.rhemify.gateway.plist"
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.rhemify.gateway"])
            ),
        )
...
class TestSystemUnitRhemifyHome:
    """RHEMIFY_HOME in system units must reference the target user, not root."""

    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        # Simulate sudo: Path.home() returns /root, target user is alice
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("RHEMIFY_HOME", raising=False)
        monkeypatch.setattr(
        ...
        assert 'RHEMIFY_HOME=/home/alice/.rhemify' in unit
        assert '/root/.rhemify' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: RHEMIFY_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("RHEMIFY_HOME", "/root/.rhemify/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'RHEMIFY_HOME=/home/alice/.rhemify/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_rhemify_home(self, monkeypatch):
        # Custom RHEMIFY_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("RHEMIFY_HOME", "/opt/rhemify-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'RHEMIFY_HOME=/opt/rhemify-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's RHEMIFY_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        rhemify_home = str(gateway_cli.get_rhemify_home().resolve())
        assert f'RHEMIFY_HOME={rhemify_home}' in unit


class TestRhemifyHomeForTargetUser:
    """Unit tests for _rhemify_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("RHEMIFY_HOME", raising=False)

        result = gateway_cli._rhemify_home_for_target_user("/home/alice")
        assert result == "/home/alice/.rhemify"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._rhemify_home_for_target_user("/home/alice")
        assert result == "/home/alice/.rhemify/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("RHEMIFY_HOME", "/opt/rhemify")

        result = gateway_cli._rhemify_home_for_target_user("/home/alice")
        assert result == "/opt/rhemify"
