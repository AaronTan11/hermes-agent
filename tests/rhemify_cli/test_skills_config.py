"""Tests for rhemify_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from rhemify_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("rhemify_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from rhemify_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("rhemify_cli.config.load_config")
    ...
    @patch("rhemify_cli.config.load_config")
    @patch.dict("os.environ", {"RHEMIFY_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("RHEMIFY_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """RHEMIFY_SESSION_PLATFORM should be used when RHEMIFY_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
        ...
        monkeypatch.setenv("RHEMIFY_HOME", str(tmp_path))
        monkeypatch.delenv("RHEMIFY_PLATFORM", raising=False)
        monkeypatch.setenv("RHEMIFY_SESSION_PLATFORM", "discord")

        from agent.skill_utils import get_disabled_skill_names
        result = get_disabled_skill_names()
        assert result == {"discord-skill"}

    def test_rhemify_platform_takes_precedence(self, tmp_path, monkeypatch):
        """RHEMIFY_PLATFORM should win over RHEMIFY_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("RHEMIFY_HOME", str(tmp_path))
        monkeypatch.setenv("RHEMIFY_PLATFORM", "telegram")
        monkeypatch.setenv("RHEMIFY_SESSION_PLATFORM", "discord")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from rhemify_cli.skills_config import _get_categories
        skills = [
