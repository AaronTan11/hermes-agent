"""Tests for rhemify_cli.tools_config platform tool persistence."""

from unittest.mock import patch

from rhemify_cli.tools_config import (
    _configure_provider,
    _get_platform_tools,
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/rhemify/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("rhemify_cli.tools_config.save_config"):
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (rhemify-cli, rhemify-telegram, etc.) must NOT
    ...
    ``rhemify tools``, but rhemify-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "rhemify-cli",  # <-- the culprit
    ...
    # rhemify-cli must NOT survive — it's a platform default, not an MCP server
    assert "rhemify-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_rhemify_telegram():
    """Same bug for Telegram — rhemify-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "rhemify-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "rhemify-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "rhemify-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    config = {"model": {"provider": "unused"}}

    monkeypatch.setattr(
        "rhemify_cli.nous_subscription.get_provider_auth_status",
        lambda: {"logged_in": True},
    )

    providers = _visible_providers(TOOL_CATEGORIES["browser"], config)

    assert providers[0]["name"].startswith("Subscription")
...
def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    ...
    assert all(not provider["name"].startswith("Subscription") for provider in providers)
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("rhemify_cli.tools_config._run_post_setup", lambda key: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "rhemify_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web", "image_gen", "tts", "browser"},
    )
    monkeypatch.setattr("rhemify_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "rhemify_cli.nous_subscription.get_provider_auth_status",
        lambda: {"logged_in": True},
    )
    ...
    configured = []
    monkeypatch.setattr(
        "rhemify_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from rhemify_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """rhemify-gateway includes list should cover all messaging platforms."""
        from rhemify_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["rhemify-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from rhemify_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from rhemify_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
