"""Tests for provider-aware `/model` validation in rhemify_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("rhemify_cli.models.fetch_api_models", return_value=api_models), \
         patch("rhemify_cli.models.probe_api_models", return_value=probe_payload):
...
class TestParseModelInput:
    ...
    def test_provider_colon_model_switches_provider(self):
        provider, model = parse_model_input("openrouter:anthropic/claude-sonnet-4.5", "unused")
        assert provider == "openrouter"
        assert model == "anthropic/claude-sonnet-4.5"
    ...
    def test_nous_provider_switch(self):
        provider, model = parse_model_input("nous:rhemify-3", "openrouter")
        assert provider == "unused"
        assert model == "rhemify-3"

    def test_empty_model_after_colon_keeps_current(self):
        provider, model = parse_model_input("openrouter:", "unused")
        assert provider == "unused"
...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("rhemify_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("rhemify_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_copilot_acp_falls_back_to_copilot_defaults(self):
        with patch("rhemify_cli.auth.resolve_api_key_provider_credentials", side_effect=Exception("no token")), \
             patch("rhemify_cli.models._fetch_github_models", return_value=None):
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("rhemify_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("rhemify_cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("rhemify_cli.models.urllib.request.urlopen", return_value=_Resp()) as mock_urlopen:
    ...
    def test_fetch_github_model_catalog_filters_non_chat_models(self):
        ...
        with patch("rhemify_cli.models.urllib.request.urlopen", return_value=_Resp()):
...
class TestCopilotNormalization:
    ...
    def test_copilot_api_mode_gpt5_uses_responses(self):
        """GPT-5+ models should use Responses API (matching upstream)."""
    ...
    def test_normalize_upstream_model_id_strips_provider_prefix(self):
        assert normalize_upstream_model_id("upstream-go", "upstream-go/kimi-k2.5") == "kimi-k2.5"
        assert normalize_upstream_model_id("upstream-zen", "upstream-zen/claude-sonnet-4-6") == "claude-sonnet-4-6"
        assert normalize_upstream_model_id("upstream-go", "glm-5") == "glm-5"

    def test_upstream_zen_api_modes_match_docs(self):
        assert upstream_model_api_mode("upstream-zen", "gpt-5.4") == "codex_responses"
        assert upstream_model_api_mode("upstream-zen", "gpt-5.3-codex") == "codex_responses"
        assert upstream_model_api_mode("upstream-zen", "upstream-zen/gpt-5.4") == "codex_responses"
        assert upstream_model_api_mode("upstream-zen", "claude-sonnet-4-6") == "anthropic_messages"
        assert upstream_model_api_mode("upstream-zen", "upstream-zen/claude-sonnet-4-6") == "anthropic_messages"
        assert upstream_model_api_mode("upstream-zen", "gemini-3-flash") == "chat_completions"
        assert upstream_model_api_mode("upstream-zen", "minimax-m2.5") == "chat_completions"

    def test_upstream_go_api_modes_match_docs(self):
        assert upstream_model_api_mode("upstream-go", "glm-5") == "chat_completions"
        assert upstream_model_api_mode("upstream-go", "upstream-go/glm-5") == "chat_completions"
        assert upstream_model_api_mode("upstream-go", "kimi-k2.5") == "chat_completions"
        assert upstream_model_api_mode("upstream-go", "upstream-go/kimi-k2.5") == "chat_completions"
        assert upstream_model_api_mode("upstream-go", "minimax-m2.5") == "anthropic_messages"
        assert upstream_model_api_mode("upstream-go", "upstream-go/minimax-m2.5") == "anthropic_messages"
