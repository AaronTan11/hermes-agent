"""Tests for the update check mechanism in rhemify_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from rhemify_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path):
    ...
    with patch("rhemify_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("rhemify_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path):
    ...
    with patch("rhemify_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("rhemify_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
            result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path):
    ...
    import rhemify_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "rhemify_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root():
    """Dev install: falls back to Path(__file__).parent.parent when RHEMIFY_HOME has no git repo."""
    import rhemify_cli.banner as banner
    ...
    # Point RHEMIFY_HOME at a temp dir with no rhemify/.git
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        with patch("rhemify_cli.banner.os.getenv", return_value=td):
            with patch("rhemify_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from rhemify_cli.main import _invalidate_update_cache

    # Build a fake ~/.rhemify with default + two named profiles
