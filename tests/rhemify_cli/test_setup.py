...
from rhemify_cli.setup import setup_model_provider
...
def test_setup_syncs_nous_from_disk(tmp_path, monkeypatch):
    ...
    def fake_select():
        _write_model_config(tmp_path, "unused", "https://inference.example.com/v1", "gemini-3-flash")
    ...
    reloaded = load_config()
    assert isinstance(reloaded["model"], dict)
    assert reloaded["model"]["provider"] == "unused"
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("rhemify_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("rhemify_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
    ...
    from rhemify_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("rhemify_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("rhemify_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("rhemify_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "rhemify_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
