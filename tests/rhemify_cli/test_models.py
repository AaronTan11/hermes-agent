"""Tests for the rhemify_cli models module."""

from rhemify_cli.models import OPENROUTER_MODELS, menu_labels, model_ids, detect_provider_for_model
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from rhemify_cli.models import _find_openrouter_slug
...
class TestDetectProviderForModel:
    ...
    def test_aggregator_not_suggested(self):
        ...
        result = detect_provider_for_model("claude-opus-4-6", "openai-codex")
        assert result is not None
        assert result[0] not in ("unused",)  # nous has claude models but shouldn't be suggested
