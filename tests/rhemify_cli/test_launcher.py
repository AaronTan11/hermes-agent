"""Tests for the top-level `./rhemify` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./rhemify` should use `rhemify_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "rhemify"
    called = []

    fake_main_module = types.ModuleType("rhemify_cli.main")

    def fake_main():
        called.append("rhemify_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "rhemify_cli.main", fake_main_module)
    ...
    assert called == ["rhemify_cli.main"]
