"""Tests for rhemify_cli.status model/provider display."""
...
def _patch_common_status_deps(monkeypatch, status_mod, tmp_path, *, openai_base_url=""):
    ...
    monkeypatch.setattr(status_mod, "get_rhemify_home", lambda: tmp_path, raising=False)
    ...
    monkeypatch.setattr(auth_mod, "get_provider_auth_status", lambda: {}, raising=False)
...
def test_show_status_displays_configured_dict_model_and_provider_label(monkeypatch, capsys, tmp_path):
    from rhemify_cli import status as status_mod
...
def test_show_status_reports_managed_nous_features(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(
        status_mod,
        "load_config",
        lambda: {"model": {"default": "claude-opus-4-6", "provider": "unused"}},
        raising=False,
    )
    monkeypatch.setattr(status_mod, "resolve_requested_provider", lambda requested=None: "unused", raising=False)
    monkeypatch.setattr(status_mod, "resolve_provider", lambda requested=None, **kwargs: "unused", raising=False)
    ...
    out = capsys.readouterr().out
    assert "Subscription Features" in out
    assert "Browser automation" in out
    assert "active via subscription" in out
...
def test_show_status_hides_nous_subscription_section_when_feature_flag_is_off(monkeypatch, capsys, tmp_path):
    ...
    out = capsys.readouterr().out
    assert "Subscription Features" not in out
