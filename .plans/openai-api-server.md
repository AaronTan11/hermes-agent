# OpenAI-Compatible API Server for Rhemify
...
SSE streaming. By exposing this endpoint, rhemify becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.rhemify/.env`
2. Run `rhemify gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with rhemify through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns rhemify as a model) |
...
Authorization: Bearer rhemify-api-key-here
...
But rhemify sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "rhemify"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured rhemify model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of rhemify's core prompt
...
Once implemented, rhemify works as a drop-in backend for:
