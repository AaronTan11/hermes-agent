...
The PID file lives at ``{RHEMIFY_HOME}/gateway.pid``.  RHEMIFY_HOME defaults to
``~/.rhemify`` but can be overridden via the environment variable.  This means
separate RHEMIFY_HOME directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "rhemify-gateway"
_RUNTIME_STATUS_FILE = "gateway_state.json"
_LOCKS_DIRNAME = "gateway-locks"


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting RHEMIFY_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("RHEMIFY_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "rhemify" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Rhemify gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different RHEMIFY_HOME dirs).
    """
