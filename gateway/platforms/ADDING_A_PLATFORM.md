...
Checklist for integrating a new messaging platform into the Rhemify gateway.
...
And add it to the `rhemify-gateway` composite:

```python
"rhemify-gateway": {
    "includes": [..., "rhemify-your-platform"]
}
```
...
## 12. Status Display (`rhemify_cli/status.py`)
...
## 13. Gateway Setup Wizard (`rhemify_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ rhemify_cli/ toolsets.py \
  --include="*.py" -l | sort -u
