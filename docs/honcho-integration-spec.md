...
Comparison of Rhemify vs. upstream-honcho — and a porting spec for bringing Rhemify patterns into other Honcho integrations.

---

## Overview

Two independent Honcho integrations have been built for two different agent runtimes: **Rhemify** (Python, baked into the runner) and **upstream-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of Rhemify-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in Rhemify that are worth propagating and patterns in upstream-honcho that Rhemify should eventually adopt. The spec is additive, not prescriptive.

---

## Architecture comparison

### Rhemify: baked-in runner
...
| Dimension | Rhemify | upstream-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "Rhemify" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from Rhemify are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.
...
**upstream-honcho contributes back (Rhemify should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with Rhemify

Already done in upstream-honcho (do not re-implement): `lastSavedIndex` dedup, platform metadata stripping, multi-agent parent observer, `peerPerspective` on `context()`, tiered tool surface, workspace `agentPeerMap`, QMD passthrough, self-hosted Honcho.

---

## nanobot-honcho checklist

Greenfield integration. Start from upstream-honcho's architecture and apply all Rhemify patterns from day one.
