...
VERSION_FILE = REPO_ROOT / "rhemify_cli" / "__init__.py"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/rhemify",
    ...
    lines.append(f"# Rhemify v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Rhemify. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Rhemify Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Rhemify Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Rhemify v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Rhemify v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Rhemify v{new_version} ({calver_date})' "
