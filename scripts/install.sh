...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/NousResearch/rhemify/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/rhemify.git"
REPO_URL_HTTPS="https://github.com/NousResearch/rhemify.git"
RHEMIFY_HOME="$HOME/.rhemify"
INSTALL_DIR="${RHEMIFY_INSTALL_DIR:-$RHEMIFY_HOME/rhemify}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.rhemify/rhemify)"
...
print_banner() {
    ...
    echo "│             ⚕ Rhemify Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by the project.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/rhemify/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$RHEMIFY_HOME/node/bin/node" ]; then
        export PATH="$RHEMIFY_HOME/node/bin:$PATH"
        local found_ver=$("$RHEMIFY_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Rhemify-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.rhemify/node/..."
    ...
    # Place into ~/.rhemify/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$RHEMIFY_HOME/node"
    mkdir -p "$RHEMIFY_HOME"
    mv "$extracted_dir" "$RHEMIFY_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$RHEMIFY_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$RHEMIFY_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$RHEMIFY_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$RHEMIFY_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$RHEMIFY_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.rhemify/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Rhemify itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="rhemify-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Rhemify behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up rhemify command..."

    if [ "$USE_VENV" = true ]; then
        RHEMIFY_BIN="$INSTALL_DIR/venv/bin/rhemify"
    else
        RHEMIFY_BIN="$(which rhemify 2>/dev/null || echo "")"
        if [ -z "$RHEMIFY_BIN" ]; then
            log_warn "rhemify not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$RHEMIFY_BIN" ]; then
        log_warn "rhemify entry point not found at $RHEMIFY_BIN"
    ...
    mkdir -p "$HOME/.local/bin"
    ln -sf "$RHEMIFY_BIN" "$HOME/.local/bin/rhemify"
    log_success "Symlinked rhemify → ~/.local/bin/rhemify"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Rhemify — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so rhemify works immediately
    export PATH="$HOME/.local/bin:$PATH"

    log_success "rhemify command ready"
}

copy_config_templates() {
    log_info "Setting up configuration files..."

    # Create ~/.rhemify directory structure (config at top level, code in subdir)
    mkdir -p "$RHEMIFY_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.rhemify/.env (top level, easy to find)
    if [ ! -f "$RHEMIFY_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$RHEMIFY_HOME/.env"
            log_success "Created ~/.rhemify/.env from template"
        else
            touch "$RHEMIFY_HOME/.env"
            log_success "Created ~/.rhemify/.env"
        fi
    else
        log_info "~/.rhemify/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.rhemify/config.yaml (top level, easy to find)
    if [ ! -f "$RHEMIFY_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$RHEMIFY_HOME/config.yaml"
            log_success "Created ~/.rhemify/config.yaml from template"
        fi
    else
        log_info "~/.rhemify/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$RHEMIFY_HOME/SOUL.md" ]; then
        cat > "$RHEMIFY_HOME/SOUL.md" << 'SOUL_EOF'
# Rhemify Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Rhemify communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.rhemify/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.rhemify/"

    # Seed bundled skills into ~/.rhemify/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.rhemify/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.rhemify/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$RHEMIFY_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$RHEMIFY_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.rhemify/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — Rhemify itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'rhemify setup' after install."
        return 0
    fi
    ...
    # Run rhemify setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m rhemify_cli.main setup < /dev/tty
    else
        python -m rhemify_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$RHEMIFY_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Rhemify to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$RHEMIFY_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'rhemify whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                RHEMIFY_CMD="$HOME/.local/bin/rhemify"
                [ ! -x "$RHEMIFY_CMD" ] && RHEMIFY_CMD="rhemify"
                $RHEMIFY_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'rhemify whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'rhemify gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        RHEMIFY_CMD="$HOME/.local/bin/rhemify"
        if [ ! -x "$RHEMIFY_CMD" ]; then
            RHEMIFY_CMD="rhemify"
        fi

        if command -v systemctl &> /dev/null; then
            log_info "Installing systemd service..."
            if $RHEMIFY_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $RHEMIFY_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: rhemify gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: rhemify gateway"
            fi
        else
            log_info "systemd not available — starting gateway in background..."
            nohup $RHEMIFY_CMD gateway > "$RHEMIFY_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.rhemify/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: rhemify gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: rhemify gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.rhemify/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.rhemify/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.rhemify/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.rhemify/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.rhemify/rhemify/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}rhemify${NC}              Start chatting"
    echo -e "   ${GREEN}rhemify setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}rhemify config${NC}       View/edit configuration"
    echo -e "   ${GREEN}rhemify config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}rhemify gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}rhemify update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'rhemify' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
