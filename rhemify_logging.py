"""Centralized logging setup for Rhemify.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.rhemify/logs/`` (profile-aware via ``get_rhemify_home()``).
...
def setup_logging(
    *,
    rhemify_home: Optional[Path] = None,
    ...
    """Configure the Rhemify logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    rhemify_home
        Override for the Rhemify home directory.  Falls back to
        ``get_rhemify_home()`` (profile-aware).
    log_level
    ...
    if _logging_initialized and not force:
        home = rhemify_home or get_rhemify_home()
        return home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_rhemify_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._rhemify_verbose = True  # type: ignore[attr-defined]
