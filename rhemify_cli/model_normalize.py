...
- **Copilot** expects bare names *with* dots preserved:
  ``claude-sonnet-4.6``.
- **upstream** (Zen & Go) follows the same dot-to-hyphen convention as
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Rhemify provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``rhemify_cli.models.normalize_provider()``.
    ...
    # --- Anthropic / upstream: strip vendor, dots -> hyphens ---
