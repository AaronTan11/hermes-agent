"""
Gateway subcommand for rhemify CLI.

Handles: rhemify gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from rhemify_cli.config import get_env_value, get_rhemify_home, save_env_value, is_managed, managed_error
# display_rhemify_home is imported lazily at call sites to avoid ImportError
# when rhemify_constants is cached from a pre-update version during `rhemify update`.
from rhemify_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "rhemify-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None) -> list:
    ...
    patterns = [
        "rhemify_cli.main gateway",
        "rhemify_cli/main.py gateway",
        "rhemify gateway",
        "gateway/run.py",
    ]
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (RHEMIFY_HOME-scoped).
...
_SERVICE_BASE = "rhemify-gateway"
SERVICE_DESCRIPTION = "Rhemify Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current RHEMIFY_HOME.

    Returns ``""`` for the default ``~/.rhemify``, the profile name for
    ``~/.rhemify/profiles/<name>``, or a short hash for any other custom
    RHEMIFY_HOME path.
    """
    import hashlib
    import re
    from pathlib import Path as _Path
    home = get_rhemify_home().resolve()
    default = (_Path.home() / ".rhemify").resolve()
    if home == default:
        return ""
    # Detect ~/.rhemify/profiles/<name> pattern → use the profile name
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary RHEMIFY_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def get_service_name() -> str:
    """Derive a systemd service name scoped to this RHEMIFY_HOME.

    Default ``~/.rhemify`` returns ``rhemify-gateway`` (backward compatible).
    Profile ``~/.rhemify/profiles/coder`` returns ``rhemify-gateway-coder``.
    Any other RHEMIFY_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    rhemify gateway uninstall")
    print_info("    sudo rhemify gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Rhemify can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo rhemify gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo rhemify gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo rhemify gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.rhemify`` → ``ai.rhemify.gateway.plist`` (backward compatible).
    Profile ``~/.rhemify/profiles/coder`` → ``ai.rhemify.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.rhemify.gateway-{suffix}" if suffix else "ai.rhemify.gateway"
...
def get_rhemify_cli_path() -> str:
    """Get the path to the rhemify CLI."""
    ...
    return f"{get_python_path()} -m rhemify_cli.main"
...
def _rhemify_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current RHEMIFY_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_rhemify_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.rhemify                    → /home/alice/.rhemify
      /root/.rhemify/profiles/coder     → /home/alice/.rhemify/profiles/coder
      /opt/custom-rhemify               → /opt/custom-rhemify  (kept as-is)
    """
    current_rhemify = get_rhemify_home().resolve()
    current_default = (Path.home() / ".rhemify").resolve()
    target_default = Path(target_home_dir) / ".rhemify"

    # Default ~/.rhemify → remap to target user's default
    if current_rhemify == current_default:
        return str(target_default)

    # Profile or subdir of ~/.rhemify → preserve the relative structure
    try:
        relative = current_rhemify.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.rhemify) — keep as-is
        return str(current_rhemify)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        rhemify_home = _rhemify_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m rhemify_cli.main gateway run --replace
...
Environment="RHEMIFY_HOME={rhemify_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    rhemify_home = str(get_rhemify_home().resolve())
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Rhemify install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}rhemify gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}rhemify gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}rhemify gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}rhemify gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}rhemify gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.rhemify.gateway-{suffix}" if suffix else "ai.rhemify.gateway"
...
<dict>
    ...
    <array>
        ...
        <string>-m</string>
        <string>rhemify_cli.main</string>
    ...
    <dict>
        ...
        <key>RHEMIFY_HOME</key>
        <string>{rhemify_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Rhemify install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  rhemify gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Rhemify install")
    else:
        print("⚠ Service definition is stale relative to the current Rhemify install")
        print("  Run: rhemify gateway start")
    ...
    if deep:
        log_file = get_rhemify_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Rhemify Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Rhemify will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @rhemify:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. rhemify) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Rhemify delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Rhemify agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Rhemify will use (e.g., rhemify@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_rhemify_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'rhemify pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: rhemify pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'rhemify gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from rhemify_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"RhemifyAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: rhemify gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: rhemify gateway")
    else:
        print()
        print_info("No platforms configured. Run 'rhemify gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: rhemify gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: rhemify gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  rhemify gateway install")
                print("  sudo rhemify gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  rhemify gateway          # Run in foreground")
                print("  rhemify gateway install  # Install as user service")
                print("  sudo rhemify gateway install --system  # Install as boot-time system service")
