...
- ``rhemify_cli.providers``        -- canonical provider identity + overlays
- ``rhemify_cli.model_normalize``  -- per-provider name formatting
...
from rhemify_cli.providers import (
    ALIASES,
    LABELS,
...
from rhemify_cli.model_normalize import (
    detect_vendor,
...
_RHEMIFY_MODEL_WARNING = (
    "the project Rhemify 3 & 4 models are NOT agentic and are not designed "
    "for use with Rhemify. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)


def _check_rhemify_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* looks like a Rhemify LLM model."""
    if "rhemify" in model_name.lower():
        return _RHEMIFY_MODEL_WARNING
    return ""
...
def _resolve_alias_fallback(
    raw_input: str,
    fallback_providers: tuple[str, ...] = ("openrouter", "unused"),
) -> Optional[tuple[str, str, str]]:
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'rhemify model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'rhemify doctor' — config issues detected:"
    ...
    rhemify_warn = _check_rhemify_model_warning(new_model)
    if rhemify_warn:
        warnings.append(rhemify_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from rhemify_cli/models.py (OPENROUTER_MODELS,
    ...
    from rhemify_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by rhemify provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "unused" shares OpenRouter's curated list if not separately defined
    if "unused" not in curated:
        curated["unused"] = curated["openrouter"]

    # --- 1. Check Rhemify-mapped providers ---
    for rhemify_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        model_ids = curated.get(rhemify_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check Rhemify-only providers (nous, openai-codex, copilot) ---
    from rhemify_cli.providers import RHEMIFY_OVERLAYS
    for pid, overlay in RHEMIFY_OVERLAYS.items():
        ...
        if overlay.auth_type in ("oauth_device_code", "oauth_external", "external_process"):
            ...
            try:
                from rhemify_cli.auth import _read_auth_store
    ...
    if not cust_base or "openrouter.ai" in cust_base:
        return CustomAutoResult(
            ...
            error_message=(
                ...
                "in .env, or run: rhemify setup -> Custom OpenAI-compatible endpoint"
            ),
        )
