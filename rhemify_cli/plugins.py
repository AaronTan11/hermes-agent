"""
Rhemify Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   – ``~/.rhemify/plugins/<name>/``
2. **Project plugins** – ``./.rhemify/plugins/<name>/`` (opt-in via
   ``RHEMIFY_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``rhemify_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "rhemify_agent.plugins"

_NS_PARENT = "rhemify_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``rhemify honcho ...``).
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.rhemify/plugins/)
        rhemify_home = os.environ.get("RHEMIFY_HOME", os.path.expanduser("~/.rhemify"))
        user_dir = Path(rhemify_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.rhemify/plugins/)
        if _env_enabled("RHEMIFY_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".rhemify" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``rhemify_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``rhemify tools`` TUI so plugin-provided toolsets appear
