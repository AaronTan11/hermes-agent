"""
Profile management for multiple isolated Rhemify instances.

Each profile is a fully independent RHEMIFY_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.rhemify/profiles/<name>/`` by default.

The "default" profile is ``~/.rhemify`` itself — backward compatible,
zero migration needed.

Usage::

    rhemify profile create coder          # fresh profile + bundled skills
    rhemify profile create coder --clone  # also copy config, .env, SOUL.md
    rhemify profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    rhemify -p coder chat                 # or via flag
    rhemify profile use coder             # set as sticky default
    rhemify profile delete coder          # remove profile + alias + service
"""
...
# Directories/files to exclude when exporting the default (~/.rhemify) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "rhemify",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "rhemify", "default", "test", "tmp", "root", "sudo",
})

# Rhemify subcommands that cannot be used as profile names/aliases
_RHEMIFY_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.rhemify/profiles/`` — anchored to the user's home,
    NOT to the current RHEMIFY_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".rhemify" / "profiles"


def _get_default_rhemify_home() -> Path:
    """Return the default (pre-profile) RHEMIFY_HOME path."""
    return Path.home() / ".rhemify"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_rhemify_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.rhemify
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its RHEMIFY_HOME directory."""
    if name == "default":
        return _get_default_rhemify_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, rhemify subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _RHEMIFY_SUBCOMMANDS:
        return f"'{name}' conflicts with a rhemify subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_rhemify_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.rhemify)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches RHEMIFY_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "RHEMIFY_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.rhemify).\n"
            "To remove everything, use: rhemify uninstall"
        )
    ...
    # Check for service
    from rhemify_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set RHEMIFY_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("RHEMIFY_HOME")
    try:
        os.environ["RHEMIFY_HOME"] = str(profile_dir)
        from rhemify_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["RHEMIFY_HOME"] = old_home
        elif "RHEMIFY_HOME" in os.environ:
            del os.environ["RHEMIFY_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.rhemify/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: rhemify profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from RHEMIFY_HOME.

    Returns ``"default"`` if RHEMIFY_HOME is not set or points to ``~/.rhemify``.
    Returns the profile name if RHEMIFY_HOME points into ``~/.rhemify/profiles/<name>``.
    Returns ``"custom"`` if RHEMIFY_HOME is set to an unrecognized path.
    """
    from rhemify_constants import get_rhemify_home
    rhemify_home = get_rhemify_home()
    resolved = rhemify_home.resolve()

    default_resolved = _get_default_rhemify_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.rhemify itself — its parent is ~/ and its
        # directory name is ".rhemify", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: rhemify profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.rhemify itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.rhemify). "
            "Specify a different name: rhemify profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for rhemify profile names."""
    return '''# Rhemify profile completion
# Add to ~/.bashrc: eval "$(rhemify completion bash)"

_rhemify_profiles() {
    local profiles_dir="$HOME/.rhemify/profiles"
...
_rhemify_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_rhemify_profiles)" -- "$cur"))
        return
    fi
...
complete -F _rhemify_completion rhemify
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for rhemify profile names."""
    return '''#compdef rhemify
# Rhemify profile completion
# Add to ~/.zshrc: eval "$(rhemify completion zsh)"

_rhemify() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.rhemify/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.rhemify/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a RHEMIFY_HOME path string.

    Called early in the CLI entry point, before any rhemify modules
    are imported, to set the RHEMIFY_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: rhemify profile create {profile_name}"
        )
