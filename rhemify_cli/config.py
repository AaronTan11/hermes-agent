"""
Configuration management for Rhemify.

Config files are stored in ~/.rhemify/ for easy access:
- ~/.rhemify/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.rhemify/.env         - API keys and secrets

This module provides:
- rhemify config          - Show current configuration
- rhemify config edit     - Open config in editor
- rhemify config set      - Set a specific value
- rhemify config wizard   - Re-run setup wizard
"""
...
from tools.tool_backend_helpers import managed_tools_enabled as _managed_tools_enabled
...
from rhemify_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("RHEMIFY_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_rhemify_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Rhemify is running in package-manager-managed mode.

    Two signals: the RHEMIFY_MANAGED env var (set by the systemd service),
    or a .managed marker file in RHEMIFY_HOME (set by the NixOS activation
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "rhemify update"


def format_managed_message(action: str = "modify this Rhemify installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("RHEMIFY_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Rhemify installation is managed by NixOS "
            f"(RHEMIFY_MANAGED={env_hint}).\n"
            "Edit services.rhemify.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Rhemify installation is managed by Homebrew "
            f"(RHEMIFY_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade rhemify"
        )

    return (
        f"Cannot {action}: this Rhemify installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Rhemify."
    )
...
# Re-export from rhemify_constants — canonical definition lives there.
from rhemify_constants import get_rhemify_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_rhemify_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_rhemify_home() / ".env"
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into RHEMIFY_HOME if the user doesn't have one yet."""
...
def ensure_rhemify_home():
    """Ensure ~/.rhemify directory structure exists with secure permissions."""
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Rhemify
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Rhemify sends a stable profile-scoped userId to Camofox
    ...
    # always goes to ~/.rhemify/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for rhemify-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Rhemify*" header.
    ...
    # Logging — controls file logging to ~/.rhemify/logs/.
...
# selection step (Provider / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        ...
        "prompt": "Provider base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Rhemify auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @rhemify:example.org)",
    ...
    # RHEMIFY_TOOL_PROGRESS and RHEMIFY_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "RHEMIFY_TOOL_PROGRESS": {
    ...
    "RHEMIFY_EPHEMERAL_SYSTEM_PROMPT": {
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'rhemify setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Rhemify won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'rhemify doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("RHEMIFY_TOOL_PROGRESS")
            old_mode = get_env_value("RHEMIFY_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from RHEMIFY_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from RHEMIFY_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("RHEMIFY_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from RHEMIFY_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: rhemify config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.rhemify/config.yaml."""
    import copy
    ensure_rhemify_home()
...
#   openai-codex (OAuth — rhemify login) — OpenAI Codex
#   nous         (OAuth — rhemify login) — Provider
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.rhemify/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.rhemify/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Rhemify env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.rhemify/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.rhemify/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.rhemify/.env and os.environ.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.rhemify/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Rhemify Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  rhemify config edit     # Edit config file", Colors.DIM))
    print(color("  rhemify config set <key> <value>", Colors.DIM))
    print(color("  rhemify setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: rhemify config set <key> <value>")
            print()
            print("Examples:")
            print("  rhemify config set model anthropic/claude-sonnet-4")
            print("  rhemify config set terminal.backend docker")
            print("  rhemify config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'rhemify config migrate' to add them")
        ...
        print("  rhemify config           Show current configuration")
        print("  rhemify config edit      Open config in editor")
        print("  rhemify config set <key> <value>   Set a config value")
        print("  rhemify config check     Check for missing/outdated config")
        print("  rhemify config migrate   Update config with new options")
        print("  rhemify config path      Show config file path")
        print("  rhemify config env-path  Show .env file path")
        sys.exit(1)
