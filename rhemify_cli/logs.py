"""``rhemify logs`` — view and filter Rhemify log files.

Supports tailing, following, session filtering, level filtering, and
relative time ranges.  All log files live under ``~/.rhemify/logs/``.

Usage examples::

    rhemify logs                    # last 50 lines of agent.log
    rhemify logs -f                 # follow agent.log in real time
    rhemify logs errors             # last 50 lines of errors.log
    rhemify logs gateway -n 100     # last 100 lines of gateway.log
    rhemify logs --level WARNING    # only WARNING+ lines
    rhemify logs --session abc123   # filter by session ID substring
    rhemify logs --since 1h         # lines from the last hour
    rhemify logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_rhemify_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Rhemify runs — try 'rhemify chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_rhemify_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_rhemify_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_rhemify_home()}/logs/")
        return

    print(f"Log files in {display_rhemify_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'rhemify chat' to generate logs)")
