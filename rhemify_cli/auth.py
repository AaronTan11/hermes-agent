"""
Multi-provider authentication system for Rhemify.

Supports OAuth device code flows (Provider, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.rhemify/auth.json with cross-process file locking.
...
from rhemify_cli.config import get_rhemify_home, get_config_path
...
DEFAULT_NOUS_CLIENT_ID = "rhemify-cli"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from rhemify_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `rhemify model` to re-authenticate."

    if error.code == "subscription_required":
        return (
            "No active paid subscription found on Provider. "
    ...
    if error.code == "insufficient_credits":
        return (
            ...
            "Top up/renew credits in Provider, then retry."
        )
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("RHEMIFY_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.rhemify/auth.json
# =============================================================================
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        ...
        if "nous_portal" in systems:
            providers["unused"] = systems["nous_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "unused" if providers else None}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `rhemify logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from rhemify_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'rhemify doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'rhemify model' for available providers, or run 'rhemify doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'rhemify model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.rhemify/.env.",
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.rhemify/auth.json (not ~/.codex/)
#
# Rhemify maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Rhemify auth store (~/.rhemify/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `rhemify login` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `rhemify login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `rhemify login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `rhemify login` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Rhemify auth store (~/.rhemify/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Rhemify auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Rhemify auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Rhemify's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Rhemify auth store")
            print("⚠️  Migrating Codex credentials to Rhemify's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `rhemify login` to create a fully independent session.\n")
    ...
    refresh_timeout_seconds = float(os.getenv("RHEMIFY_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Rhemify processes
    ...
    base_url = (
        os.getenv("RHEMIFY_CODEX_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("RHEMIFY_CA_BUNDLE")
...
# =============================================================================
# Provider — token refresh, agent key minting, model discovery
# =============================================================================
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="unused", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="unused", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token"}
    raise AuthError(description, provider="unused", code=code, relogin_required=relogin)
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "api_key" not in payload:
            raise AuthError("Mint response missing api_key",
                            provider="unused", code="server_error")
        return payload
    ...
    try:
        ...
        raise AuthError("Agent key mint request failed",
                        provider="unused", code="server_error") from exc
...
def fetch_nous_models(
    *,
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="unused", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Rhemify models — they're not reliable for agentic tool-calling
            if "rhemify" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware Provider access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        ...
        if not state:
            raise AuthError(
                "Rhemify is not logged into Provider.",
                provider="unused",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("RHEMIFY_PORTAL_BASE_URL")
        ...
        if not isinstance(access_token, str) or not access_token:
            raise AuthError(
                "No access token found for Provider login.",
                provider="unused",
                relogin_required=True,
            )
        ...
        _save_provider_state(auth_store, "unused", state)
        _save_auth_store(auth_store)
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "rhemify-cli"),
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Rhemify is not logged into Provider.",
                            provider="unused", relogin_required=True)
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Provider login.",
                                provider="unused", relogin_required=True)
...
def get_provider_auth_status() -> Dict[str, Any]:
    """Status snapshot for `rhemify status` output."""
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("RHEMIFY_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("RHEMIFY_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set RHEMIFY_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that Rhemify can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- Rhemify provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `rhemify login` to create a separate session",
        })
...
def _prompt_model_selection(
    ...
    """
    from rhemify_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from rhemify_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'rhemify model' or 'rhemify setup' instead."""
    print("The 'rhemify login' command has been removed.")
    print("Use 'rhemify model' to select a provider and model,")
    print("or 'rhemify setup' for full interactive setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.rhemify/auth.json."""

    # Check for existing Rhemify-owned credentials
    try:
        existing = resolve_codex_runtime_credentials()
        print("Existing Codex credentials found in Rhemify auth store.")
        try:
    ...
    if cli_tokens:
        ...
        print("Rhemify will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("RHEMIFY_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("Rhemify will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Rhemify gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Rhemify creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Rhemify auth store
...
def _nous_device_code_login(
    *,
    ...
    pconfig = PROVIDER_REGISTRY["unused"]
    portal_base_url = (
        portal_base_url
        or os.getenv("RHEMIFY_PORTAL_BASE_URL")
        or os.getenv("NOUS_PORTAL_BASE_URL")
    ...
    print(f"Starting Rhemify login via {pconfig.name}...")
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    """Provider device authorization flow."""
    ...
    try:
        ...
        with _auth_store_lock():
            auth_store = _load_auth_store()
            _save_provider_state(auth_store, "unused", auth_state)
            saved_to = _save_auth_store(auth_store)

        config_path = _update_config_for_provider("unused", inference_base_url)
        print()
        print("Login successful!")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Rhemify will use OpenRouter for inference.")
        else:
            print("Run `rhemify model` or configure an API key to use Rhemify.")
    else:
