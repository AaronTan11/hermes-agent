"""
Doctor command for rhemify CLI.

Diagnoses issues with Rhemify setup.
"""
...
from rhemify_cli.config import get_project_root, get_rhemify_home, get_env_path
from rhemify_constants import display_rhemify_home

PROJECT_ROOT = get_project_root()
RHEMIFY_HOME = get_rhemify_home()
_DHH = display_rhemify_home()  # user-facing display path (e.g. ~/.rhemify or ~/.rhemify/profiles/coder)

# Load environment variables from ~/.rhemify/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.rhemify/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `rhemify`.
    os.environ.setdefault("RHEMIFY_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Rhemify Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.rhemify/.env (primary location for user config)
    env_path = RHEMIFY_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'rhemify setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'rhemify setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'rhemify setup' to create one")
                issues.append("Run 'rhemify setup' to create .env")
    
    # Check ~/.rhemify/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        try:
            from rhemify_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'rhemify setup' to migrate config")
                else:
                    issues.append("Run 'rhemify doctor --fix' or 'rhemify setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'rhemify doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from rhemify_cli.auth import get_provider_auth_status, get_codex_auth_status

        provider_status = get_provider_auth_status()
        if provider_status.get("logged_in"):
            check_ok("Provider auth", "(logged in)")
        else:
            check_warn("Provider auth", "(not logged in)")
    ...
    rhemify_home = RHEMIFY_HOME
    if rhemify_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            rhemify_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = rhemify_home / subdir_name
    ...
    soul_path = rhemify_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Rhemify a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Rhemify Persona\n\n"
                "<!-- Edit this file to customize how Rhemify communicates. -->\n\n"
                "You are Rhemify, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = rhemify_home / "memories"
    ...
    state_db_path = rhemify_home / "state.db"
    ...
    wal_path = rhemify_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'rhemify doctor --fix' to checkpoint")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'rhemify setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = RHEMIFY_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: rhemify skills list)")
    ...
    try:
        ...
        if not _honcho_cfg_path.exists():
            check_warn("Honcho config not found", "run: rhemify memory setup")
        elif not hcfg.enabled:
            check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
        elif not (hcfg.api_key or hcfg.base_url):
            check_fail("Honcho API key or base URL not set", "run: rhemify memory setup")
            issues.append("No Honcho API key — run 'rhemify memory setup'")
        else:
    ...
    try:
        ...
        if mem0_key:
            ...
            mem0_json = RHEMIFY_HOME / "mem0.json"
            if mem0_json.exists():
                try:
                    import json as _json
            ...
            check_warn("Mem0 not configured", "(set MEM0_API_KEY in .env or run rhemify memory setup)")
    except ImportError:
    ...
    # =========================================================================
    try:
        from rhemify_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "rhemify -p" in content:
                            _m = _re.search(r"rhemify -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'rhemify doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
