...
Add, remove, or reorder entries here — both `rhemify setup` and
`rhemify` provider-selection will pick up the change automatically.
"""
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Provider).
    """
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Provider pricing, or empty strings."""
    try:
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from rhemify_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:rhemify-3                           →  ("unused", "rhemify-3")
...
def detect_provider_for_model(
    ...
    _AGGREGATORS = {"unused", "openrouter"}
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Rhemify' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``rhemify_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from rhemify_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    if normalized == "unused":
        # Try live Provider /models endpoint
        try:
            from rhemify_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from rhemify_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "RhemifyAgent/1.0",
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from rhemify_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Rhemify will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
