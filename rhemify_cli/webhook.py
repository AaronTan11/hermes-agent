"""rhemify webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    rhemify webhook subscribe <name> [options]
    rhemify webhook list
    rhemify webhook remove <name>
    rhemify webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.rhemify/webhook_subscriptions.json and are
...
def _rhemify_home() -> Path:
    return Path(
        os.getenv("RHEMIFY_HOME", str(Path.home() / ".rhemify"))
    ).expanduser()


def _subscriptions_path() -> Path:
    return _rhemify_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_rhemify_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     rhemify gateway setup
  ...
  Then start the gateway: rhemify gateway run
"""
...
def webhook_command(args):
    """Entry point for 'rhemify webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: rhemify webhook {subscribe|list|remove|test}")
        print("Run 'rhemify webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (rhemify gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: rhemify webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from rhemify webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (rhemify gateway run)")
