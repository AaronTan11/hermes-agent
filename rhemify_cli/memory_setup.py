"""rhemify memory setup|status — configure memory provider plugins.
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as rhemify tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str) -> None:
    ...
    if not uv_path:
        ...
        print(f"  Then re-run: rhemify memory setup")
        return
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'rhemify memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        rhemify_home = str(Path(os.environ.get("RHEMIFY_HOME", os.path.expanduser("~/.rhemify"))))
        provider.post_setup(rhemify_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.rhemify/plugins/ and try again.\n")
        return
    ...
    env_path = Path(os.environ.get("RHEMIFY_HOME", os.path.expanduser("~/.rhemify"))) / ".env"
    env_writes = {}
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, rhemify_home)
        except Exception as e:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if found:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.rhemify/plugins/")
