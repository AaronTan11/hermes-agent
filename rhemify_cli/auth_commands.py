...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "unused", "openai-codex"}
...
def auth_add_command(args) -> None:
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = AUTH_TYPE_OAUTH if provider in {"anthropic", "unused", "openai-codex"} else AUTH_TYPE_API_KEY
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_rhemify_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:rhemify_pkce",
    ...
    raise SystemExit(f"`rhemify auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from rhemify_cli.config import remove_env_value
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `rhemify auth` is called bare."""
