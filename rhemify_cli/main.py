#!/usr/bin/env python3
"""
Rhemify CLI - Main entry point.

Usage:
    rhemify                     # Interactive chat (default)
    rhemify chat                # Interactive chat
    rhemify gateway             # Run gateway in foreground
    rhemify gateway start       # Start gateway as service
    rhemify gateway stop        # Stop gateway service
    rhemify gateway status      # Show gateway status
    rhemify gateway install     # Install gateway service
    rhemify gateway uninstall   # Uninstall gateway service
    rhemify setup               # Interactive setup wizard
    rhemify logout              # Clear stored authentication
    rhemify status              # Show status of all components
    rhemify cron                # Manage cron jobs
    rhemify cron list           # List cron jobs
    rhemify cron status         # Check if cron scheduler is running
    rhemify doctor              # Check configuration and dependencies
    rhemify honcho setup                    # Configure Honcho AI memory integration
    rhemify honcho status                   # Show Honcho config and connection status
    rhemify honcho sessions                 # List directory → session name mappings
    rhemify honcho map <name>               # Map current directory to a session name
    rhemify honcho peer                     # Show peer names and dialectic settings
    rhemify honcho peer --user NAME         # Set user peer name
    rhemify honcho peer --ai NAME           # Set AI peer name
    rhemify honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    rhemify honcho mode                     # Show current memory mode
    rhemify honcho mode [hybrid|honcho|local]  # Set memory mode
    rhemify honcho tokens                   # Show token budget settings
    rhemify honcho tokens --context N       # Set session.context() token cap
    rhemify honcho tokens --dialectic N     # Set dialectic result char cap
    rhemify honcho identity                 # Show AI peer identity representation
    rhemify honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    rhemify honcho migrate                  # Step-by-step migration guide: upstream native → Rhemify + Honcho
    rhemify version             Show version
    rhemify update              Update to latest version
    rhemify uninstall           Uninstall Rhemify
    rhemify acp                 Run as an ACP server for editor integration
    rhemify sessions browse     Interactive session picker with search

    rhemify claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (rhemify tools, rhemify setup, rhemify model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'rhemify {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any rhemify module import.
#
# Many modules cache RHEMIFY_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("RHEMIFY_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.rhemify/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set RHEMIFY_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check ~/.rhemify/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".rhemify" / "active_profile"
    ...
    # 3. If we found a profile, resolve and set RHEMIFY_HOME
    if profile_name is not None:
        try:
            from rhemify_cli.profiles import resolve_profile_env
            rhemify_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent rhemify from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["RHEMIFY_HOME"] = rhemify_home
...
load_rhemify_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `rhemify` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from rhemify_logging import setup_logging as _setup_logging
...
from rhemify_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from rhemify_cli.config import get_env_path, get_rhemify_home, load_config
    from rhemify_cli.auth import get_auth_status

    # Determine whether Rhemify itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from rhemify_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_rhemify_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from rhemify_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for Provider OAuth credentials
    auth_file = get_rhemify_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Rhemify has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Rhemify to use their tokens.
    if _has_rhemify_config:
        try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'rhemify sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Rhemify isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  rhemify setup")
        print()

        from rhemify_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'rhemify setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from rhemify_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["RHEMIFY_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from rhemify_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from rhemify_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Rhemify?")
        print()
    ...
    session_dir = get_rhemify_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: rhemify gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  rhemify gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Rhemify'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  rhemify gateway")
        ...
        print()
        print("  Or install as a service: rhemify gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'rhemify whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    from rhemify_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``rhemify model``) and the setup wizard
    ...
    from rhemify_cli.config import load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("RHEMIFY_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    providers = [
        ...
        ("unused", "Provider (the project subscription)"),
        ("openai-codex", "OpenAI Codex"),
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from rhemify_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from rhemify_cli.config import get_env_value, save_env_value
    ...
    from rhemify_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    """Provider provider: ensure logged in, then pick model."""
    from rhemify_cli.auth import (
    ...
    from rhemify_cli.config import get_env_value, save_config, save_env_value
    from rhemify_cli.nous_subscription import (
        apply_nous_provider_defaults,
        get_nous_subscription_explainer_lines,
    )
    import argparse

    state = get_provider_auth_state("unused")
    if not state or not state.get("access_token"):
        print("Not logged into Provider. Starting login...")
        print()
        try:
            ...
            _login_nous(mock_args, PROVIDER_REGISTRY["unused"])
            print()
    ...
    from rhemify_cli.models import _PROVIDER_MODELS, get_pricing_for_provider
    model_ids = _PROVIDER_MODELS.get("unused", [])
    if not model_ids:
        print("No curated models available for Provider.")
        return
    ...
    try:
        ...
        if relogin:
            ...
            print("Re-authenticating with Provider...\n")
            try:
                mock_args = argparse.Namespace(
    ...
    # Fetch live pricing (non-blocking — returns empty dict on failure)
    pricing = get_pricing_for_provider("unused")
    ...
    if selected:
        ...
        _update_config_for_provider("unused", inference_url)
        ...
        save_config(config)
        print(f"Default model set to: {selected} (via Provider)")
        if "tts" in changed_defaults:
            print("TTS provider set to: OpenAI TTS via your subscription")
        else:
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from rhemify_cli.codex_models import get_codex_model_ids
    from rhemify_cli.config import get_env_value, save_env_value
    import argparse
...
def _model_flow_custom(config):
    ...
    """
    from rhemify_cli.auth import _save_model_choice, deactivate_provider
    ...
    from rhemify_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `rhemify model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from rhemify_cli.auth import _save_model_choice, deactivate_provider
    from rhemify_cli.config import save_env_value, load_config, save_config
    from rhemify_cli.models import fetch_api_models
...
from rhemify_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from rhemify_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from rhemify_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Rhemify turns to `copilot --acp`.")
    print("  Rhemify currently starts its own ACP subprocess for each request.")
    print("  Rhemify uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set RHEMIFY_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    """Generic flow for API-key providers (z.ai, MiniMax, upstream, etc.)."""
    from rhemify_cli.auth import (
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from rhemify_cli.config import (
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(f"    Rhemify will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'rhemify model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Rhemify CLI with a provider."""
    from rhemify_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from rhemify_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from rhemify_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from rhemify_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from rhemify_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from rhemify_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from rhemify_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from rhemify_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Rhemify v{__version__} ({__release_date__})")
    ...
    try:
        from rhemify_cli.banner import check_for_updates
        from rhemify_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from rhemify_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``rhemify update --gateway`` so interactive prompts (stash restore,
...
def _update_via_zip(args):
    """Update Rhemify by downloading a ZIP archive.
    ...
    try:
        tmp_dir = tempfile.mkdtemp(prefix="rhemify-update-")
        zip_path = os.path.join(tmp_dir, f"rhemify-{branch}.zip")
        ...
        # GitHub ZIPs extract to rhemify-<branch>/
        extracted = os.path.join(tmp_dir, f"rhemify-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("rhemify-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Rhemify behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so Rhemify can run?")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Rhemify couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Rhemify couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Rhemify behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `rhemify update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from rhemify_constants import get_rhemify_home
    return (get_rhemify_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from rhemify_constants import get_rhemify_home
        (get_rhemify_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``rhemify update``, every profile is now current.
    """
    homes = []
    # Default profile home
    default_home = Path.home() / ".rhemify"
    homes.append(default_home)
    # Named profiles under ~/.rhemify/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "rhemify[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Rhemify to the latest version."""
    import shutil
    from rhemify_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Rhemify")
        return
    ...
    try:
        ...
        # the old bytecode (e.g. get_rhemify_home added to rhemify_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload rhemify_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_rhemify_home() added since the last release.
        try:
            import importlib
            import rhemify_constants as _hc
        ...
        try:
            from rhemify_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'rhemify config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: rhemify config migrate")
            else:
                print()
                print("Skipped. Run 'rhemify config migrate' later to configure.")
        else:
        ...
        try:
            from rhemify_cli.gateway import (
            ...
            # Discover all rhemify-gateway* units (default + profiles)
            if is_linux():
                try:
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "rhemify-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. rhemify-gateway.service or rhemify-gateway-coder.service
            ...
            if is_macos():
                try:
                    from rhemify_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: rhemify gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: rhemify -p <profile> gateway run  for each profile)")
        ...
        print("  rhemify model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``rhemify -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from rhemify_cli.profiles import (
    ...
    from rhemify_constants import display_rhemify_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `rhemify profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_rhemify_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → rhemify -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.rhemify)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  rhemify profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     rhemify -p {name} chat")
                else:
            ...
            if clone or clone_all:
                from rhemify_constants import get_rhemify_home
                profile_dir_display = f"~/.rhemify/profiles/{name}"
        ...
        name = args.profile_name
        from rhemify_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from rhemify_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from rhemify_cli.profiles import rename_profile
        try:
        ...
        from rhemify_cli.profiles import export_profile
        name = args.profile_name
        ...
        from rhemify_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from rhemify_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def cmd_logs(args):
    """View and filter Rhemify log files."""
    from rhemify_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for rhemify CLI."""
    parser = argparse.ArgumentParser(
        prog="rhemify",
        description="Rhemify - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    rhemify                        Start interactive chat
    rhemify chat -q "Hello"        Single query mode
    rhemify -c                     Resume the most recent session
    rhemify -c "my project"        Resume a session by name (latest in lineage)
    rhemify --resume <session_id>  Resume a specific session by ID
    rhemify setup                  Run setup wizard
    rhemify logout                 Clear stored authentication
    rhemify auth add <provider>    Add a pooled credential
    rhemify auth list              List pooled credentials
    rhemify auth remove <p> <t>    Remove pooled credential by index, id, or label
    rhemify auth reset <provider>  Clear exhaustion status for a provider
    rhemify model                  Select default model
    rhemify config                 View configuration
    rhemify config edit            Edit config in $EDITOR
    rhemify config set model gpt-4 Set a config value
    rhemify gateway                Run messaging gateway
    rhemify -s rhemify-dev,github-auth
    rhemify -w                     Start in isolated git worktree
    rhemify gateway install        Install gateway background service
    rhemify sessions list          List past sessions
    rhemify sessions browse        Interactive session picker
    rhemify sessions rename ID T   Rename/title a session
    rhemify logs                   View agent.log (last 50 lines)
    rhemify logs -f                Follow agent.log in real time
    rhemify logs errors            View errors.log
    rhemify logs --since 1h        Lines from the last hour
    rhemify update                 Update to latest version

For more help on a command:
    rhemify <command> --help
"""
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Rhemify"
    )
    ...
    chat_parser.add_argument(
        "--provider",
        choices=["auto", "openrouter", "unused", "openai-codex", "copilot-acp", "copilot", "anthropic", "huggingface", "zai", "kimi-coding", "minimax", "minimax-cn", "kilocode"],
        default=None,
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: rhemify-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Rhemify with an interactive wizard. "
                    "Run a specific section: rhemify setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Rhemify CLI"
    )
    login_parser.add_argument(
        "--provider",
        choices=["unused", "openai-codex"],
        default=None,
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: rhemify-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Rhemify components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Rhemify setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Rhemify configuration"
    )
    ...
    def cmd_pairing(args):
        from rhemify_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from rhemify_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from rhemify_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/rhemify-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from rhemify_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from rhemify_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'rhemify tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # rhemify tools list [--platform cli]
    ...
    # rhemify tools disable <name...> [--platform cli]
    ...
    # rhemify tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from rhemify_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from rhemify_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Rhemify as an MCP server",
        description=(
            "Manage MCP server connections and run Rhemify as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'rhemify mcp add' to connect to a new server, or\n"
            "'rhemify mcp serve' to expose Rhemify conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Rhemify as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from rhemify_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch rhemify --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            rhemify_bin = shutil.which("rhemify")
            if rhemify_bin:
                os.execvp(rhemify_bin, ["rhemify", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "rhemify_cli.main", "--resume", selected_id],
                )
    ...
    # =========================================================================
    # claw command (upstream migration)
    # =========================================================================
    claw_parser = subparsers.add_parser(
        "claw",
        help="upstream migration tools",
        description="Migrate settings, memories, skills, and API keys from upstream to Rhemify"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from upstream to Rhemify",
        description="Import settings, memories, skills, and API keys from an upstream installation"
    )
    claw_migrate.add_argument(
        "--source",
        help="Path to upstream directory (default: ~/.upstream)"
    )
    claw_migrate.add_argument(
        "--dry-run",
        action="store_true",
    ...
    claw_cleanup = claw_subparsers.add_parser(
        "cleanup",
        aliases=["clean"],
        help="Archive leftover upstream directories after migration",
        description="Scan for and archive leftover upstream directories to prevent state fragmentation"
    )
    claw_cleanup.add_argument(
        "--source",
        help="Path to a specific upstream directory to clean up"
    )
    claw_cleanup.add_argument(
        "--dry-run",
        action="store_true",
    ...
    def cmd_claw(args):
        from rhemify_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Rhemify to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Rhemify",
        description="Remove Rhemify from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Rhemify as an ACP (Agent Client Protocol) server",
        description="Start Rhemify in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Rhemify as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Rhemify instances",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Rhemify log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    rhemify logs                    Show last 50 lines of agent.log
    rhemify logs -f                 Follow agent.log in real time
    rhemify logs errors             Show last 50 lines of errors.log
    rhemify logs gateway -n 100     Show last 100 lines of gateway.log
    rhemify logs --level WARNING    Only show WARNING and above
    rhemify logs --session abc123   Filter by session ID
    rhemify logs --since 1h         Lines from the last hour
    rhemify logs --since 30m -f     Follow, starting from 30 min ago
    rhemify logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``rhemify -c Pokemon Agent Dev`` → ``rhemify -c 'Pokemon Agent Dev'``
