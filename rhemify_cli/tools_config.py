"""
Unified tool configuration for Rhemify.

`rhemify tools` and `rhemify setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.rhemify/config.yaml under
...
# They're still in _RHEMIFY_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from rhemify_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "rhemify-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "rhemify-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "rhemify-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "rhemify-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "rhemify-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "rhemify-signal"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "rhemify-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "rhemify-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "rhemify-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "rhemify-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "rhemify-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "rhemify-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "rhemify-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "rhemify-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "rhemify-webhook"},
}
...
TOOL_CATEGORIES = {
    ...
    "browser": {
        ...
        "providers": [
            {
                "name": "Subscription (Browserbase cloud)",
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_rhemify_home()}/rhemify && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in rhemify directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "rhemify-cli" (which include all _RHEMIFY_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "rhemify-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `rhemify tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by rhemify tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (rhemify-cli, rhemify-telegram, etc.)
...
def _configure_provider(provider: dict, config: dict):
    ...
    if provider.get("requires_nous_auth"):
        ...
        if not features.nous_auth_present:
            _print_warning("  Subscription is only available after logging into Provider.")
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your subscription.")
            override_envs = provider.get("override_env_vars", [])
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.rhemify/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `rhemify tools` and `rhemify setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Rhemify Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://rhemify.local/docs/user-guide/features/tools", Colors.DIM))
    print()
        ...
        for pkey in enabled_platforms:
            ...
            if managed_tools_enabled():
                for ts_key in sorted(auto_configured):
                    ...
                    print(color(f"  ✓ {label}: using your subscription defaults", Colors.GREEN))
    ...
    print()
    from rhemify_constants import display_rhemify_home
    print(color(f"  Tool configuration saved to {display_rhemify_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'rhemify' or gateway restart.", Colors.DIM))
    print()
