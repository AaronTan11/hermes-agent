"""Slash command definitions and autocomplete for the Rhemify CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("paste", "Check clipboard for an image and attach it", "Info",
               cli_only=True),
    CommandDef("update", "Update Rhemify to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    try:
        ...
        config_path = os.path.join(
            os.getenv("RHEMIFY_HOME", os.path.expanduser("~/.rhemify")),
            "config.yaml",
        )
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``rhemify skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /rhemify handler.

    Maps both canonical names and aliases so /rhemify bg do stuff works
    the same as /rhemify background do stuff.
    """
    overrides = _resolve_config_gates()
