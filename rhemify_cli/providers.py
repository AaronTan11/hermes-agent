"""
Single source of truth for provider identity in Rhemify.
...
2. **Rhemify overlays** — transport type, auth patterns, aggregator flags,
...
# -- Rhemify overlay ----------------------------------------------------------
# Rhemify-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class RhemifyOverlay:
    """Rhemify-specific provider metadata layered on top of models.dev."""
...
RHEMIFY_OVERLAYS: Dict[str, RhemifyOverlay] = {
    "openrouter": RhemifyOverlay(
        transport="openai_chat",
        is_aggregator=True,
        extra_env_vars=("OPENAI_API_KEY",),
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "unused": RhemifyOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.example.com/v1",
    ),
    "openai-codex": RhemifyOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "copilot-acp": RhemifyOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": RhemifyOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": RhemifyOverlay(
    ...
    "kimi-for-coding": RhemifyOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": RhemifyOverlay(
        transport="openai_chat",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": RhemifyOverlay(
        transport="openai_chat",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": RhemifyOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": RhemifyOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "vercel": RhemifyOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "rhemify", "user-config"
...
def get_overlay(provider_id: str) -> Optional[RhemifyOverlay]:
    """Get Rhemify overlay for a provider, if one exists."""
    canonical = normalize_provider(provider_id)
    return RHEMIFY_OVERLAYS.get(canonical)
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Rhemify overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Rhemify overlay
    ...
    overlay = RHEMIFY_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + rhemify extra
    ...
    if overlay is not None:
        # Rhemify-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
LABELS: Dict[str, str] = {
    ...
    # Legacy Rhemify IDs (point to same providers)
