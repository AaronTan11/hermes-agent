"""
Interactive setup wizard for Rhemify.
...
Config files are stored in ~/.rhemify/ for easy access.
"""
...
from rhemify_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://rhemify.local/docs"
...
_DEFAULT_PROVIDER_MODELS = {
    ...
    "upstream-zen": ["gpt-5.4", "gpt-5.3-codex", "claude-sonnet-4-6", "gemini-3-flash", "glm-5", "kimi-k2.5", "minimax-m2.7"],
    "upstream-go": ["glm-5", "kimi-k2.5", "mimo-v2-pro", "mimo-v2-omni", "minimax-m2.5", "minimax-m2.7"],
    "huggingface": [
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from rhemify_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    from rhemify_cli.config import get_env_value
    ...
    if provider_id in {"upstream-zen", "upstream-go"}:
        provider_models = [normalize_upstream_model_id(provider_id, mid) for mid in provider_models]
    ...
    if model_idx < len(provider_models):
        ...
        elif provider_id in {"upstream-zen", "upstream-go"}:
            selected_model = normalize_upstream_model_id(provider_id, selected_model)
        ...
        if custom:
            if is_copilot_catalog_provider:
                ...
                selected_model = normalize_upstream_model_id(provider_id, custom)
            else:
    ...
    elif provider_id in {"upstream-zen", "upstream-go"} and selected_model:
        model_cfg = _model_config_dict(config)
        model_cfg["api_mode"] = upstream_model_api_mode(provider_id, selected_model)
        config["model"] = model_cfg
...
# display_rhemify_home imported lazily at call sites (stale-module safety during rhemify update)

from rhemify_cli.colors import Colors, color
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Rhemify Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Rhemify using environment variables or config commands:")
    print_info("  rhemify config set model.provider custom")
    print_info("  rhemify config set model.base_url http://localhost:8080/v1")
    print_info("  rhemify config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'rhemify setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from rhemify_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'rhemify setup')")


def _print_setup_summary(config: dict, rhemify_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'rhemify setup' to configure"))
    ...
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (subscription)", True, None))
    ...
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (subscription)", True, None))
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'rhemify setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (subscription)", True, None))
    elif config.get("terminal", {}).get("backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'rhemify setup terminal'"))
    elif managed_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via subscription)", True, None))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'rhemify setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {rhemify_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('rhemify setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('rhemify setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('rhemify setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('rhemify setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('rhemify setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('rhemify config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('rhemify config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('rhemify config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('rhemify', Colors.GREEN)}              Start chatting")
    print(f"   {color('rhemify gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('rhemify doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `rhemify tools` and `rhemify setup tools`).
...
def setup_model_provider(config: dict):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``rhemify model``)
    ...
    provider added to ``rhemify model`` is automatically available here.
    """
    ...
    # Delegate to the shared rhemify model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from rhemify_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: rhemify model")
    ...
    nous_subscription_selected = selected_provider == "unused"
    ...
    if _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from rhemify_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Rhemify can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Rhemify can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'rhemify setup' or configure AUXILIARY_VISION_* settings")


    if selected_provider == "unused" and nous_subscription_selected:
        ...
        if "tts" in changed_defaults:
            print_success("TTS provider set to: OpenAI TTS via your subscription")
        else:
    ...
    # subscription defaults are already being applied.
    if selected_provider != "unused":
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.rhemify/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'rhemify setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Rhemify runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Rhemify via Telegram/Discord, this is where")
        print_info(
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("RHEMIFY_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("RHEMIFY_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Rhemify from anywhere.")
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where Rhemify delivers cron job results,")
    ...
    if not existing_matrix and prompt_yes_no("Set up Matrix?", False):
        ...
        if token or get_env_value("MATRIX_PASSWORD"):
            ...
            # Home room
            print()
            print_info("📬 Home Room: where Rhemify delivers cron job results and notifications.")
    ...
    if not existing_mattermost and prompt_yes_no("Set up Mattermost?", False):
        ...
        if token:
            ...
            # Home channel
            print()
            print_info("📬 Home Channel: where Rhemify delivers cron job results and notifications.")
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'rhemify whatsapp' for guided setup.")
        print_info(f"   Full guide: {_DOCS_BASE}/user-guide/messaging/whatsapp")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'rhemify whatsapp' to choose your mode (separate bot number")
    ...
    if not existing_webhook and prompt_yes_no("Set up webhooks? (GitHub, GitLab, etc.)", False):
        ...
        print()
        print_info("   Open config in your editor:  rhemify config edit")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     rhemify config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: rhemify gateway install")
            else:
                print_info("  You can install later: rhemify gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo rhemify gateway install --system")
                print_info("  Or run in foreground:  rhemify gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   rhemify gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `rhemify setup tools` and `rhemify tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    Used after upstream migration to detect which sections can be skipped.
    ``get_env_value`` is the module-level import from rhemify_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from rhemify_cli.auth import get_active_provider
...
def _offer_upstream_migration(rhemify_home: Path) -> bool:
    """Migration is disabled in this deployment."""
    return False
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      rhemify setup           — full or quick (auto-detected)
      rhemify setup model     — just model/provider
      rhemify setup terminal  — just terminal backend
      rhemify setup gateway   — just messaging platforms
      rhemify setup tools     — just tool configuration
      rhemify setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Rhemify Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Rhemify Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Rhemify installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Rhemify configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, rhemify_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'rhemify setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'rhemify setup' again when ready.")
            return
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, rhemify_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        # Offer upstream migration before configuration begins
        migration_ran = _offer_upstream_migration(rhemify_home)
        if migration_ran:
    ...
    print_info(f"Data folder:  {rhemify_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'rhemify config edit'")

    if migration_ran:
        print()
        print_info("Settings were imported from upstream.")
...
def _run_quick_setup(config: dict, rhemify_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'rhemify setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Rhemify to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'rhemify setup gateway'.")
