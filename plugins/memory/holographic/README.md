...
```bash
rhemify memory setup    # select "holographic"
```

Or manually:
```bash
rhemify config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.rhemify-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$RHEMIFY_HOME/memory_store.db` | SQLite database path |
