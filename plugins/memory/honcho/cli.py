...
Handles: rhemify honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "rhemify" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Rhemify profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from rhemify_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'rhemify honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'rhemify honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `rhemify update` -- no output, no exceptions.
    """
    try:
        from rhemify_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Rhemify profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $RHEMIFY_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_rhemify_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Rhemify persistent cross-session memory.")
    ...
    rhemify_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'rhemify honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = rhemify_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        rhemify_host["peerName"] = new_peer

    current_ai = rhemify_host.get("aiPeer") or cfg.get("aiPeer", "rhemify")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        rhemify_host["aiPeer"] = new_ai

    current_workspace = rhemify_host.get("workspace") or cfg.get("workspace", "rhemify")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        rhemify_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = rhemify_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        rhemify_host["observationMode"] = new_obs
    else:
        rhemify_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(rhemify_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        rhemify_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        rhemify_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = rhemify_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        rhemify_host["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = rhemify_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        rhemify_host["sessionStrategy"] = new_strat

    rhemify_host["enabled"] = True
    rhemify_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        rhemify_config = load_config()
        rhemify_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(rhemify_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: rhemify config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    rhemify honcho status     -- show full config")
    print("    rhemify honcho mode       -- change recall/observation mode")
    print("    rhemify honcho tokens     -- tune context and dialectic budgets")
    print("    rhemify honcho peer       -- update peer names")
    print("    rhemify honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Rhemify profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: rhemify honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'rhemify honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: rhemify honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        rhemify = hosts.get(_host_key(), {})
        user = rhemify.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = rhemify.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = rhemify.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = rhemify.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Rhemify' identity in Honcho. Seed with 'rhemify honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: rhemify honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = rhemify.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = rhemify.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = rhemify.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Rhemify asks Honcho's AI peer a question")
        ...
        print("\n  Set with: rhemify honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'rhemify honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'rhemify honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    rhemify honcho identity --show        — show both peer representations")
        print("    rhemify honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: upstream native memory → Rhemify + Honcho."""
    from pathlib import Path

    # ── Detect upstream native memory files ──────────────────────────────────
    cwd = Path(os.getcwd())
    upstream_home = Path.home() / ".upstream"
    ...
    print("\nHoncho migration: upstream native memory → Rhemify\n" + "─" * 50)
    print()
    print("  upstream's native memory stores context in local markdown files")
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Rhemify persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  rhemify honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'rhemify honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'rhemify honcho setup' when ready, then re-run this walkthrough.")

    # ── Step 2: Detected files ────────────────────────────────────────────────
    print()
    print("Step 2  Detected upstream memory files")
    print()
    if user_files or agent_files:
        ...
        print("  No upstream native memory files found in cwd or ~/.upstream/.")
        print("  If your files are elsewhere, copy them here before continuing,")
        print("  or seed them manually:  rhemify honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'rhemify'")
        print("  with Honcho configured and no prior session history.")
        print("  (Rhemify calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    rhemify honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'rhemify honcho setup' first, then re-run this step.")
    else:
    ...
    print("  agent's character, capabilities, and behavioral rules. In upstream")
    print("  these are injected via file search at prompt-build time.")
    print()
    print("  In Rhemify, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'rhemify honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    rhemify honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  rhemify honcho identity <path/to/SOUL.md>")

    # ── Step 5: What changes ──────────────────────────────────────────────────
    print()
    print("Step 5  What changes vs. upstream native memory")
    print()
    print("  Storage")
    print("    upstream: markdown files on disk, searched via QMD at prompt-build time.")
    print("    Rhemify:   cloud-backed Honcho peers. Files can stay on disk as source")
    print("              of truth; Honcho holds the live representation.")
    print()
    print("  Context injection")
    print("    upstream: file excerpts injected synchronously before each LLM call.")
    print("    Rhemify:   Honcho context fetched async at turn end, injected next turn.")
    print("              First turn has no Honcho context; subsequent turns are loaded.")
    print()
    print("  Memory growth")
    print("    upstream: you edit files manually to update memory.")
    print("    Rhemify:   Honcho observes every message and updates representations")
    ...
    print("    upstream: no persistent session concept — files are global.")
    print("    Rhemify:   per-session by default — each run gets its own session")
    print("              Map a custom name:  rhemify honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. rhemify honcho setup              — configure API key (required)")
        print("  2. rhemify honcho migrate            — re-run this walkthrough")
    else:
        print("  1. rhemify honcho status             — verify Honcho connection")
        print("  2. rhemify                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. rhemify honcho identity --show    — verify AI peer representation")
        print("  4. rhemify honcho tokens             — tune context and dialectic budgets")
        print("  5. rhemify honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'rhemify memory setup'...\n")
        from rhemify_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``rhemify honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``rhemify honcho``.
    """
    import argparse
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to rhemify memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from upstream-honcho to Rhemify Honcho",
    )
