...
```bash
rhemify memory setup    # select "byterover"
```

Or manually:
```bash
rhemify config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.rhemify/.env
```
...
Working directory: `$RHEMIFY_HOME/byterover/` (profile-scoped).
