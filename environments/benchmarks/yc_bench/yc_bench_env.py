...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs RhemifyAgentLoop with terminal tool only
...
Requires: pip install rhemify[yc-bench]
"""
...
from environments.agent_loop import RhemifyAgentLoop
...
class YCBenchEvalConfig(RhemifyAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends RhemifyAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(RhemifyAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The RhemifyAgentLoop then drives the
    ...
    def config_init(cls) -> Tuple[YCBenchEvalConfig, List[APIServerConfig]]:
        env_config = YCBenchEvalConfig(
            ...
            steps_per_eval=1,
            total_steps=1,
            tokenizer_name="NousResearch/Rhemify-3-Llama-3.1-8B",
            use_wandb=True,
            wandb_name="yc-bench",
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs RhemifyAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our RhemifyAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the RhemifyAgentLoop
            # ----------------------------------------------------------
