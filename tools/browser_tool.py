...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-rhemify_…`` the resulting socket path exceeds the
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Rhemify-managed
    ...
    # Build an extended search PATH including Homebrew and Rhemify-managed dirs.
    ...
    rhemify_home = get_rhemify_home()
    rhemify_node_bin = str(rhemify_home / "node" / "bin")
    if os.path.isdir(rhemify_node_bin):
        extra_dirs.append(rhemify_node_bin)
...
def _run_browser_command(
    ...
    try:
        ...
        # Ensure PATH includes Rhemify-managed Node first, Homebrew versioned
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = rhemify_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_rhemify_dir("cache/screenshots", "browser_screenshots")
