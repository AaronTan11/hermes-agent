#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Rhemify Skills Hub.
...
Used by rhemify_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            rhemify_meta = metadata.get("rhemify", {})
            if isinstance(rhemify_meta, dict):
                tags = rhemify_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.rhemify/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                rhemify_meta = meta_block.get("rhemify", {})
                if isinstance(rhemify_meta, dict):
                    tags = rhemify_meta.get("tags", [])
