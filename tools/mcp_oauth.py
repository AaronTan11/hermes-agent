...
This module provides the glue:
    - ``RhemifyTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Rhemify"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses RHEMIFY_HOME so each profile gets its own OAuth tokens.
    Layout: ``RHEMIFY_HOME/mcp-tokens/``
    """
    try:
        from rhemify_constants import get_rhemify_home
        base = Path(get_rhemify_home())
    except ImportError:
        base = Path(os.environ.get("RHEMIFY_HOME", str(Path.home() / ".rhemify")))
...
# ---------------------------------------------------------------------------
# RhemifyTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class RhemifyTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        RHEMIFY_HOME/mcp-tokens/<server_name>.json         -- tokens
        RHEMIFY_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Rhemify.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = RhemifyTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Rhemify")
    scope = cfg.get("scope")
