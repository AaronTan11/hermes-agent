...
def _load_rhemify_env_vars() -> dict[str, str]:
    """Load ~/.rhemify/.env values without failing Docker command execution."""
    try:
        from rhemify_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.rhemify/sandboxes/). Non-persistent
        ...
        container_name = f"rhemify-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def execute(self, command: str, cwd: str = "", *,
        ...
        try:
            ...
            pass
        rhemify_env = _load_rhemify_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = rhemify_env.get(key)
