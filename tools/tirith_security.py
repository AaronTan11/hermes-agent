...
it is automatically downloaded from GitHub releases to $RHEMIFY_HOME/bin/tirith.
...
def _get_rhemify_home() -> str:
    """Return the Rhemify home directory, respecting RHEMIFY_HOME env var."""
    return str(get_rhemify_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_rhemify_home(), ".tirith-install-failed")
...
def _rhemify_bin_dir() -> str:
    """Return $RHEMIFY_HOME/bin, creating it if needed."""
    d = os.path.join(_get_rhemify_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $RHEMIFY_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_rhemify_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $RHEMIFY_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $RHEMIFY_HOME/bin/tirith
    ...
    rhemify_bin = os.path.join(_rhemify_bin_dir(), "tirith")
    if os.path.isfile(rhemify_bin) and os.access(rhemify_bin, os.X_OK):
        _resolved_path = rhemify_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return rhemify_bin
