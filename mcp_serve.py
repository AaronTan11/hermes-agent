"""
Rhemify MCP Server — expose messaging conversations as MCP tools.

Starts a stdio MCP server that lets any MCP client (Claude Code, Cursor, Codex,
etc.) list conversations, read message history, send messages, poll for live
events, and manage approval requests across all connected platforms.

Matches upstream's 9-tool MCP channel bridge surface:
  conversations_list, conversation_get, messages_read, attachments_fetch,
  events_poll, events_wait, messages_send, permissions_list_open,
  permissions_respond

Plus: channels_list (Rhemify-specific extra)

Usage:
    rhemify mcp serve
    rhemify mcp serve --verbose
...
logger = logging.getLogger("rhemify.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using RHEMIFY_HOME."""
    try:
        from rhemify_constants import get_rhemify_home
        return get_rhemify_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("RHEMIFY_HOME", Path.home() / ".rhemify")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from rhemify_constants import get_rhemify_home
        directory_file = get_rhemify_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("RHEMIFY_HOME", Path.home() / ".rhemify")
...
class EventBridge:
    ...
    This is the Rhemify equivalent of upstream's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from rhemify_constants import get_rhemify_home
            db_file = get_rhemify_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("RHEMIFY_HOME", Path.home() / ".rhemify")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Rhemify MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'rhemify[mcp]'"
        )

    mcp = FastMCP(
        "rhemify",
        instructions=(
            "Rhemify messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Rhemify MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
