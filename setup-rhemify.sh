...
# 5. Symlinks the 'rhemify' CLI command into ~/.local/bin
...
# ============================================================================
# PATH setup — symlink rhemify into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up rhemify command..."

RHEMIFY_BIN="$SCRIPT_DIR/venv/bin/rhemify"
mkdir -p "$HOME/.local/bin"
ln -sf "$RHEMIFY_BIN" "$HOME/.local/bin/rhemify"
echo -e "${GREEN}✓${NC} Symlinked rhemify → ~/.local/bin/rhemify"
...
# ============================================================================
# Seed bundled skills into ~/.rhemify/skills/
# ============================================================================

RHEMIFY_SKILLS_DIR="${RHEMIFY_HOME:-$HOME/.rhemify}/skills"
mkdir -p "$RHEMIFY_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.rhemify/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$RHEMIFY_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo "Other commands:"
echo "  rhemify status        # Check configuration"
echo "  rhemify gateway install # Install gateway service (messaging + cron)"
echo "  rhemify cron list     # View scheduled jobs"
echo "  rhemify doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m rhemify_cli.main setup
fi
