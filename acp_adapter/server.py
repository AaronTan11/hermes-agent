"""ACP agent server — exposes Rhemify via the Agent Client Protocol."""
...
try:
    from rhemify_cli import __version__ as RHEMIFY_VERSION
except Exception:
    RHEMIFY_VERSION = "0.0.0"
...
class RhemifyACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Rhemify AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Rhemify version",
        },
    )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["rhemify-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Rhemify using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="rhemify", version=RHEMIFY_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Rhemify on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from rhemify_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["rhemify-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Rhemify v{RHEMIFY_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Rhemify has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
