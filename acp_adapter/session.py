"""ACP session manager — maps ACP sessions to Rhemify AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.rhemify/state.db``) so they
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Rhemify agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Rhemify AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Rhemify runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.rhemify/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``RHEMIFY_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_rhemify_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=rhemify_home / "state.db")
            return self._db_instance
        except Exception:
